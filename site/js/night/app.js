...
import { prepVenues, decide, scoreVenue, pickSecond, buildCrawl, whyLine, mulberry32, hashStr, VIBES, vibeName, haversineMi, travelLabel, openState, fmtClock, DIST_DIALS } from "./engine.js?v=n15";
import { buildContext } from "./context.js?v=n15";
import { loadMemory, memoryView, setHome, toggleSaved, toggleBeen, lockDate, habitNudge, logGenerated } from "./memory.js?v=n15";
import { NightMap } from "./nightmap.js?v=n15";
import { sharePlan } from "./share.js?v=n15";
...
console.info("ChiLocal · build v=n15");
...
async function boot() {
  ...
  const [venuesRaw, geo, ctx] = await Promise.all([
    fetch("data/venues.json?v=n15").then((r) => r.json()),
    fetch("data/neighborhoods.min.geojson?v=n15").then((r) => r.json()),
    buildContext(),
  ]);
  // CTA knowledge: station list is tiny — fetch in the background, degrade silently
  fetch("data/cta-stations.min.json?v=n15").then((r) => r.json())
...
function setView(view) {
  ...
  if (view === "explore") {
    ...
    S.map.loadDetail?.("data/detail.min.geojson?v=n15");
...
function toggleOverlay(kind, force) {
  ...
  if (on) {
    if (kind === "transit") {
      S.map.loadTransit("data/cta-lines.min.geojson?v=n15");
      S.map.loadStations("data/cta-stations.min.json?v=n15");
    } else if (kind === "metra") S.map.loadMetra("data/metra-lines.min.geojson?v=n15");
    else if (kind === "divvy") S.map.loadDivvy("data/divvy-stations.min.json?v=n15");
    else S.map.loadStreets("data/streets.min.geojson?v=n15");
  }
  const prefs = loadPrefs();
